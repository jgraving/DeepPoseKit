...
class ResidualBlock:
    ...
    def __init__(self, filters, bottleneck_factor=2):
        ...
        self.identity_1x1 = conv(filters,
                                 kernel_size=(1, 1))
        
        self.bottleneck_1x1_bn = BatchNormalization()
        self.bottleneck_1x1 = conv(filters // bottleneck_factor,
                                   kernel_size=(1, 1))
        
        self.bottleneck_3x3_bn = BatchNormalization()
        self.bottleneck_3x3 = conv(filters // bottleneck_factor,
                                   kernel_size=(3, 3))
        
        self.expansion_1x1_bn = BatchNormalization()
        self.expansion_1x1 = conv(filters,
                                  kernel_size=(1, 1))
...
class FrontModule:
    def __init__(self, filters, n_downsample, bottleneck_factor=2):
        ...
        self.res_output = [ResidualBlock(filters, bottleneck_factor),
...
class Output:
    def __init__(self, n_output_channels, filters):
        ...
        self.loss_output = conv(n_output_channels,
                                activation='linear',
                                use_bias=True)
    ...
    def __call__(self, inputs):
        ...
        res_outputs = self.res_add([loss_x,
                                    conv_x,
                                    identity])
...
class Hourglass:
    def __init__(self, filters, bottleneck_factor, n_downsample, n_upsample=None):
        ...
        self.bottleneck_layers = [ResidualBlock(filters, bottleneck_factor),
                                  ResidualBlock(filters, bottleneck_factor),
                                  ResidualBlock(filters, bottleneck_factor)]
    ...
    def __call__(self, inputs):
        ...
        x = self.bottleneck_add([x_identity,
                                 x_bottleneck])
        
        
        skip_connections = skip_connections[::-1]
        
        up_layers = zip(skip_connections,
                        self.skip_res_blocks,
                        self.skip_bn_layers,
                        self.add_layers,
                        self.add_bn_layers,
                        self.up_res_blocks,
                        self.upsample_layers)
        for skip_x, skip_res_block, skip_bn, add_layer, add_bn, up_res_block, upsample_layer in up_layers:
...
class StackedHourglass(BaseModel):
    ...
    def __init__(self, data_generator, n_stacks=1,
                 n_transitions=-1, filters=256,
                 bottleneck_factor=2,
                 subpixel=True, **kwargs):
        """
    ...
    def __init_model__(self):
        ...
        max_transitions = np.min([image_utils.n_downsample(self.data_generator.height),
        ...
        for idx in range(self.n_stacks):
            ...
            outputs_x, x = Output(self.data_generator.n_output_channels, self.filters)(x)            
