...
class ConvBlock2D:
    def __init__(self, n_layers, filters, kernel_size,
                 activation, initializer='glorot_uniform',
                 batchnorm=False, use_bias=True, name=None):
    ...
    def __call__(self, inputs):
        ...
        for idx in range(self.n_layers):
            outputs = layers.Conv2D(self.filters, self.kernel_size,
                                    padding='same', activation=self.activation,
                                    use_bias=self.use_bias,
                                    kernel_initializer=self.initializer,
                                    name=self.name)(outputs)
            if self.batchnorm:
...
class ConvPool2D:
    ...
    def __call__(self, inputs):
        outputs = ConvBlock2D(self.n_layers, self.filters,
                             self.kernel_size, self.activation,
                             self.initializer, self.batchnorm,
                             self.use_bias)(inputs)
